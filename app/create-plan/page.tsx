...
// ✅ NEW: backend helper import
import { upsertProfile, ProfileInput } from "@/lib/profileService";
...
export default function FitnessFormPage() {
  ...
  // ✅ ONLY BACKEND LOGIC CHANGED HERE
  async function onSubmit(data: any) {
    setIsLoading(true);
    try {
      // 1️⃣ Build payload for Supabase RPC
      ...
      // 2️⃣ Save profile in Supabase via RPC
      await upsertProfile(profilePayload);

      // 3️⃣ Call your existing AI plan API
      const response = await fetch("/api/generate-plan", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      // 4️⃣ Store in localStorage as before
      localStorage.setItem(
        "fitnessplan",
        JSON.stringify({
      ...
      console.error(error);
      alert("Failed to save profile or generate plan. Please try again.");
    } finally {
      setIsLoading(false);
    }
  }
